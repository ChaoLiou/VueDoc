...
在這個元件中, 所有 `<google-map>` 的子孫層需要存取一個 `getMap` 方法, 為了要知道是與哪個 map 互動. 使用 `$parent` 屬性無法擴充到更深的巢狀元件. 這裡使用 DI 就會很方便, 使用兩個 instance options: `provide` 和 `inject`.

`provide` options 讓我們指定我們想要 **提供** 給子孫層元件的 data/methods. 在這個案例中, 就是 `<google-map>` 裡面的 `getMap` 方法:
...
然後在任何子孫層, 我們可以使用 `inject` option 接收我們想要加到 instance 指定的屬性:
...
元件可以在自己的 template 中遞迴呼叫自己. 不過, 他們只能在有設定 `name` option 情況下使用:
...
當你使用 `Vue.component` 全域註冊一個元件, 全域 ID 會自動設定為元件的 `name` option.
...
> 不過, `inline-template` 讓你的 template scope 難以理解原因. 最佳實作案例, 偏好是在元件裡面使用 `template` option 或在 `.vue` 檔案的 `<template>` 元素中定義 templates
